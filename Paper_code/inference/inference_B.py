"""
Inference — Config B : NLLB-200 + Full Fine-Tuning.
Traduit 20 exemples du split test → outputs/resultats_config_B.docx
"""
import sys, os
...
latest = best_checkpoint(cfg.CHECKPOINTS_B)
print(f"Meilleur checkpoint validation : {os.path.basename(latest)}")

pl_module = NLLBFullFineTuner.load_from_checkpoint(latest, cfg=cfg)
...
doc = Document()
doc.add_heading("Resultats — Config B : NLLB-200 Full Fine-Tuning", 0)
doc.add_paragraph(
    "Tous les parametres du modele NLLB-200 ont ete ajustes "
    "sur le corpus Francais-Serere."
)
...
output_path = os.path.join(cfg.OUTPUTS_DIR, "resultats_config_B.docx")
